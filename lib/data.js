// i need the node default modules for files system (fs) and path
import fs from 'fs';
...
// extract object value in filtered array if any
  ...
  if (objMatch.length > 0) {
    ...
    // we found a person, now let's try to find all snacks related to that person id
    // get filepath to json file
    const filePath2 = path.join(dataDir, 'dessertList.json');
    // load json file contents
    ...
    objReturned.dessertList = objMatch2;
